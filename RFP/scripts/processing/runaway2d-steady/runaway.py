...
class runaway:
  ...
  def readmesh(self, meshfile) :
    ...
    pw = 1.5 #min(10.*dpl, 0.1*self.xmax)
    i=0
...
#--------------------
  def output_ne(self, pc):
    ...
    hx[1:self.M] = 0.5*(self.vp1[1:self.M]-self.vp12[0:self.M-1])
...
#      ne += 2.*np.pi * np.sum(self.dist[self.My2::,M0::]*self.vp12[None,M0::]**2*zeta1(self.xip12[self.My2::], self.eps)[:,None]*hx[None,M0::]*hy[self.My2::,None])
      ne = 2.*np.pi * np.sum(self.dist[::self.My2,M0::]*hx[None,M0::]*hy[::self.My2,None]*self.vp12[None,M0::]**2)

      je = 2.*np.pi * np.sum(self.dist[:self.My2,M0::]*self.vp12[None,M0::]**3/np.sqrt(1. + self.vp12[None,M0::]**2)*self.xip12[:self.My2,None]*zeta1(self.xip12[:self.My2], self.eps)[:,None]*hx[None,M0::]*hy[:self.My2,None])
      je += 2.*np.pi * np.sum(self.dist[self.My2::,M0::]*self.vp12[None,M0::]**3/np.sqrt(1. + self.vp12[None,M0::]**2)*self.xip12[self.My2::,None]*zeta1(self.xip12[self.My2::], self.eps)[:,None]*hx[None,M0::]*hy[self.My2::,None])

      ke = 2.*np.pi * np.sum(self.dist[:self.My2,M0::]*self.vp12[None,M0::]**2*zeta1(self.xip12[:self.My2], self.eps)[:,None]*(np.sqrt(1. + self.vp12[None,M0::]**2) - 1.)*hx[None,M0::]*hy[:self.My2,None])
      ke += 2.*np.pi * np.sum(self.dist[self.My2::,M0::]*self.vp12[None,M0::]**2*zeta1(self.xip12[self.My2::], self.eps)[:,None]*(np.sqrt(1. + self.vp12[None,M0::]**2) - 1.)*hx[None,M0::]*hy[self.My2::,None])

    else :
      ne = 2.*np.pi * np.sum(self.dist[:,M0::]*hx[None,M0::]*hy[:,None]*self.vp12[None,M0::]**2)
      je = 2.*np.pi * np.sum(self.dist[:,M0::]*hx[None,M0::]*hy[:,None]*self.vp12[None,M0::]**3/np.sqrt(1. + self.vp12[None,M0::]**2)*self.xip12[:,None])
      ke = 2.*np.pi * np.sum(self.dist[:,M0::]*hx[None,M0::]*hy[:,None]*self.vp12[None,M0::]**2*(np.sqrt(self.vp12[None,M0::]**2 + 1.) -1.))
