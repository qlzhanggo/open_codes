 ...
 */
fk_equ_ba::fk_equ_ba(mesh *mesh_, char* param_file) : fk_equ(mesh_, param_file), ba_ctx(mesh_), Dw(mesh_, ba_ctx)
{
	int i, j, nn;
	PetscReal v;
	...
	if (my_mesh->with_trap) {
		...
		Ftp.resize(my_mesh->xm + 2, 0); // include the ghost points \pm 1
		...
		if (rank1 != rank2) {
			...
			if (rank == rank2) {
				...
				//std::cout<<rank2 << " receive: " << recv<<std::endl;
			}
			...
			if (rank == rank1) {
				...
				//std::cout<<rank1 << " recieve: " << recv<<std::endl;
			}
		}
	}
...
void fk_equ_ba::initialize(Field **xx, AppCtx *user)
{
	int i, j, k, nn;
	PetscReal v, xi;
 ...
 * 
 */
void fk_equ_ba::Eval(Field **xx, Field **ff, AppCtx *user)
{
	PetscInt i, j, k;
	PetscInt   nn = 0;
	...
	for (j=my_mesh->ys; j<my_mesh->ys+my_mesh->ym; j++) { //xi
		...
		for(i=my_mesh->xs; i<my_mesh->xs+my_mesh->xm; i++) { //v
			...
			if (j == my_mesh->My2) { // trap->passing (+xitp plus)
				...
				ff[j][i].fn[nn] += (Fxi(nn, i, j, xx) - Ftp2[i-my_mesh->xs+1]) /(xijp1-xij)/ba_ctx.zeta1_jp12[j];

			} else if (j == my_mesh->My1+1) { // pass->trap (-xitp plus), 

				ff[j][i].fn[nn] += (Fxi(nn, i, j, xx) - Ftp1[i-my_mesh->xs+1]) /(xijp1-xij)/ba_ctx.zeta1_jp12[j];

                //this is the old way but they should be equivalent, a bug has been fixed here though -QT
				//ff[j][i].fn[nn] += (Fxi(nn, i, j, xx) - Ftp1[i-my_mesh->xs+1]/2.) /(xijp1-xij)/ba_ctx.zeta1_jp12[j];

			} else if (j == my_mesh->My1) { // pass->trap  (-xitp minus)

                //the flux from pass to trap is determined by the conservatition of the fluxs
                //i.e.: Flux(+xitp plus)-Flux(-xitp minus) = -2*Flux(-xitp plus)
				ff[j][i].fn[nn] += (2*Ftp1[i-my_mesh->xs+1] + Ftp2[i-my_mesh->xs+1] - Fxi(nn, i, j-1, xx)) /(xijp1-xij)/ba_ctx.zeta1_jp12[j];

                //this is old way 
				//ff[j][i].fn[nn] += (Ftp1[i-my_mesh->xs+1] + Ftp2[i-my_mesh->xs+1] - Fxi(nn, i, j-1, xx)) /(xijp1-xij)/ba_ctx.zeta1_jp12[j];

			} else {

				ff[j][i].fn[nn] += (Fxi(nn, i, j, xx) - Fxi(nn, i, j-1, xx)) /(xijp1-xij)/ba_ctx.zeta1_jp12[j];
			}
...
void fk_equ_ba::compute_Fxi(Field **xx)
{
	int nn = 0;
	...
	for (int j=my_mesh->ys-1; j<my_mesh->ys+my_mesh->ym; j++) { //xi, starting one cell below to cover both ends
		...
		if (xijp1>-1. && xijp1<1. && fabs(xijp1)> 1.e-16) { // avoid boundaries
			...
			for(int i=my_mesh->xs; i<my_mesh->xs+my_mesh->xm; i++) { //v
				...
				coef = -(E/vip12) + sr/sqrt(1.+vip12*vip12)*xijp1 * (ba_ctx.zeta2_jp1[j] + 4.*ba_ctx.zeta3_jp1[j]);
				...
				if (coef < 0.) {
					...
					if (j == my_mesh->My2 || j == my_mesh->My1+1 || j == 0) { 
...
void fk_equ_ba::compute_Fv(Field **xx)
{
	int nn = 0;
	...
	for (int j=my_mesh->ys; j<my_mesh->ys+my_mesh->ym; j++) { //xi, starting one cell below to cover both ends
		...
		for(int i=my_mesh->xs-1; i<my_mesh->xs+my_mesh->xm; i++) { //v
			...
			coef = -vip1*( E*xijp12/ba_ctx.zeta1_jp12[j] + sr*vip1*sqrt(1.+vip1*vip1)*( (1. - xijp12*xijp12) + quad(rho)*vip1*vip1*quad(xijp12)) * (1. + 6.*ba_ctx.zeta4_jp12[j]/ba_ctx.zeta1_jp12[j]) );
			...
			if (i == -1) {
				...
				if (dw_ && true) {
                    ...
                    // the diffusion coefficient of this flux has to be scaled by xi_0/ba_ctx.zeta1_jp12[j], but xi_0 has been removed in the equation
...
void fk_equ_ba::compute_Ftp(Field **xx)
{
	PetscScalar coef, f(0.0), flux;
	PetscScalar vip12;

    /* This is the strict boundary matching condition, which determines the function value on the trap-passing boundary */
    /* this is old and not used any more
	double oo_dxi1 = 1./(my_mesh->yf[my_mesh->My1] - my_mesh->yy[my_mesh->My1]); // below -xitp
	double to_dxi2 = 2./(my_mesh->yy[my_mesh->My1+1] - my_mesh->yf[my_mesh->My1]); // above -xitp
	double oo_dxi3 = 1./(my_mesh->yy[my_mesh->My2] - my_mesh->yf[my_mesh->My2]); // above xitp

	//	if (my_mesh->My1>=my_mesh->ys && my_mesh->My1<(my_mesh->ys+my_mesh->ym)) 
	for(int i=my_mesh->xs; i<my_mesh->xs+my_mesh->xm; i++) {  // Ftp stores the value on the trap-passing boundary
		Ftp[i-my_mesh->xs+1] = (oo_dxi1*xx[my_mesh->My1][i].fn[0] + to_dxi2*xx[my_mesh->My1+1][i].fn[0] + oo_dxi3*xx[my_mesh->My2][i].fn[0])
											/ (oo_dxi1 + to_dxi2 + oo_dxi3);
	}
    */
...
void fk_equ_ba::SetJacobian(Field **x, Mat jac, AppCtx *user)
{
	PetscErrorCode ierr;
	int         	i,j;
...
// return non-local flux across trap-passing boundary using currently updated xx
//PetscScalar fk_equ_ba::Fxi(PetscInt nn, int i, Field **xx)
//{
//	double f, tok, coef;
//
//	// set E=0 at the trap-passing boundary
//
//	tok = ba_ctx.zeta2(ba_ctx.xic);
//	coef = sr/sqrt(1.+vip12*vip12)*ba_ctx.xic * tok;
//	//
//	if (coef>=0)
//		f = xx[my_mesh->My1][i].fn[0]; // using the upstream f value
//	else
//		f = xx[my_mesh->My2][i].fn[0];
//	//
//	f = ( coef * f  - tok * dTv[i-my_mesh->xs] * (xx[my_mesh->My2][i].fn[0] - xx[my_mesh->My1][i].fn[0])/( my_mesh->yy[my_mesh->My2] + my_mesh->yy[my_mesh->My1] ) );
//	//				ff[j][i].fn[nn] += (Fxi(nn, i, j, xx) - Ftp[i-my_mesh->xs+1]) /(xijp1-xij)/ba_ctx.zeta1(xijp12);
//
//	if (dw_) {   // with quasilinear diffusion
//		double v = vip12/sqrt(1.0+vip12*vip12);
//		double D, fluxe, fluxw;
//
//		int idx = (i-my_mesh->xs+1) + (my_mesh->My2-my_mesh->ys+1)*(my_mesh->xm+2); //use coefficient in the +1 passing region
//		//int idx2 = (i-my_mesh->xs+1) + (j+1-my_mesh->ys+1)*(my_mesh->xm+2);
//
//		// d^2f/dxi^2
//		D = Dw.xixi[idx];
//		fluxe = -(D/(vip12*vip12)) * (xx[my_mesh->My2][i].fn[0] - xx[my_mesh->My1][i].fn[0])/(my_mesh->yy[my_mesh->My2]+ my_mesh->yy[my_mesh->My1]);
//
//		// the off-diagonal coefficient
//		D = Dw.pxi[idx];
//
...
//		if (i > 0) {
//			if (D<0) {
//				fluxe -= D/vip12 * (xx[my_mesh->My2][i+1].fn[0] - xx[my_mesh->My2][i].fn[0])/(my_mesh->xx[i+1] - my_mesh->xx[i]);
//				fluxw -= D/vip12 * (xx[my_mesh->My1][i].fn[0] - xx[my_mesh->My1][i-1].fn[0])/(my_mesh->xx[i] - my_mesh->xx[i-1]);
//			} else {
//				fluxe -= D/vip12 * (xx[my_mesh->My2][i].fn[0] - xx[my_mesh->My2][i-1].fn[0])/(my_mesh->xx[i] - my_mesh->xx[i-1]);
//				fluxw -= D/vip12 * (xx[my_mesh->My1][i+1].fn[0] - xx[my_mesh->My1][i].fn[0])/(my_mesh->xx[i+1] - my_mesh->xx[i]);
//			}
//
//			if (fluxe*fluxw > 0)
//				f += 2.*fluxw* fabs(fluxe)/(fabs(fluxe)+fabs(fluxw));
//
//		} else if (i == 0) {
//			fluxe -= D/vip12 * 0.5*(xx[my_mesh->My2][i+1].fn[0] + xx[my_mesh->My1][i+1].fn[0] - xx[my_mesh->My2][i].fn[0] - xx[my_mesh->My1][i].fn[0]) / (my_mesh->xx[i+1] - my_mesh->xx[i]);
//			f += fluxe;
//		}
//
//		// convective flux from quasilinear term due to using f*p
//		if (D>0)
//			f += D/(vip12*vip12) * xx[my_mesh->My1][i].fn[0];
//		else
//			f += D/(vip12*vip12) * 0.5*(xx[my_mesh->My2][i].fn[0] + xx[my_mesh->My2][i].fn[0]);
//	}
//
//	return (1.-ba_ctx.xic*ba_ctx.xic)*f;
//}

/*-------OLD Stuff in compute_ftp -----*/
/* Below is a relaxed boundary condition by approximating the function value on the trap-passing boundary but imposes particle conservation*/
//	if (rank1 != rank2) {
//		double *temp = new double[my_mesh->xm+2];
//
//		// domain with first-trap grid send distribution data to +1 passing region
//		if (my_mesh->My1>=my_mesh->ys && my_mesh->My1<(my_mesh->ys+my_mesh->ym)) { // domain with trap-passing boundary 1{
//			for (int i=0;i<my_mesh->xm+2; i++)
//				temp[i] = xx[my_mesh->My1][i+my_mesh->xs-1].fn[0];
//			MPI_Send(temp, send, MPI_DOUBLE, rank2, 0, commy);
//
//		} else if (my_mesh->My2>=my_mesh->ys && my_mesh->My2<(my_mesh->ys+my_mesh->ym)) {// j=My2, domain with the first point of +1 passing region
//
//			MPI_Recv(Ftp.data(), recv, MPI_DOUBLE, rank1, 0, commy, MPI_STATUS_IGNORE); // received distribution on the trap side
//
//			// now setting non-local t-p flux
//			for(int i=my_mesh->xs; i<my_mesh->xs+my_mesh->xm; i++) {
//				vip12 = my_mesh->xx[i];
//				coef = -(E/vip12) + sr/sqrt(1.+vip12*vip12)*ba_ctx.xic*tok;
//
//				// Ftp stores the distribution at the first point in the trap region
//				if (coef>=0)
//					f = Ftp[i-my_mesh->xs+1];
//				else
//					f = xx[my_mesh->My2][i].fn[0];
//
//				flux = ( coef * f  - tok * dTv[i-my_mesh->xs] * (-Ftp[i-my_mesh->xs+1] + xx[my_mesh->My2][i].fn[0])/( my_mesh->yy[my_mesh->My2]+ my_mesh->yy[my_mesh->My1]) );
//
//				if (dw_) {   // with quasilinear diffusion
//					double v = vip12/sqrt(1.0+vip12*vip12);
//					double dfdp, D, f, s, fluxe, fluxw;
//
//					int idx = (i-my_mesh->xs+1) + (my_mesh->My2-my_mesh->ys+1)*(my_mesh->xm+2); // use coefficient in the +1 passing region
//					//int idx2 = (i-my_mesh->xs+1) + (j+1-my_mesh->ys+1)*(my_mesh->xm+2);
//
//					// d^2f/dxi^2
//					D = Dw.xixi[idx];
//					fluxe = -(D/(vip12*vip12)) * (-Ftp[i-my_mesh->xs+1] + xx[my_mesh->My2][i].fn[0])/(my_mesh->yy[my_mesh->My2]+ my_mesh->yy[my_mesh->My1]);
//
//					// the off-diagonal coefficient
//					D = Dw.pxi[idx];
//
...
//						if (D<0) {
//							fluxe -= D/vip12 * (xx[my_mesh->My2][i+1].fn[0] - xx[my_mesh->My2][i].fn[0])/(my_mesh->xx[i+1] - my_mesh->xx[i]);
//							fluxw -= D/vip12 * (Ftp[i-my_mesh->xs+1] - Ftp[i-my_mesh->xs])/(my_mesh->xx[i] - my_mesh->xx[i-1]);
//						} else {
//							fluxe -= D/vip12 * (xx[my_mesh->My2][i].fn[0] - xx[my_mesh->My2][i-1].fn[0])/(my_mesh->xx[i] - my_mesh->xx[i-1]);
//							fluxw -= D/vip12 * (Ftp[i-my_mesh->xs+2] - Ftp[i-my_mesh->xs+1])/(my_mesh->xx[i+1] - my_mesh->xx[i]);
//						}
//
//						if (fluxe*fluxw > 0)
//							flux += 2.*fluxw* fabs(fluxe)/(fabs(fluxe)+fabs(fluxw));
//
//					} else if (i == 0) {
//						fluxe -= D/vip12 * 0.5*(xx[my_mesh->My2][i+1].fn[0] + Ftp[i+1-my_mesh->xs+1] - xx[my_mesh->My2][i].fn[0] - Ftp[i-my_mesh->xs+1]) / (my_mesh->xx[i+1] - my_mesh->xx[i]);
//						flux += fluxe;
//					}
//
//					// convective flux from quasilinear term due to using f*p
//					if (D>0)
//						flux += D/(vip12*vip12) * Ftp[i-my_mesh->xs+1];
//					else
//						flux += D/(vip12*vip12) * 0.5*(xx[my_mesh->My2][i].fn[0] + xx[my_mesh->My2][i].fn[0]);
//				}
//
//				Ftp[i-my_mesh->xs+1] = flux * (1. - ba_ctx.xic*ba_ctx.xic);
//			}
//		}
//
//		// let's send the flux back to the first point in trap region
//		if (my_mesh->My2>=my_mesh->ys && my_mesh->My2<(my_mesh->ys+my_mesh->ym)) {
//			for (int i=0;i<my_mesh->xm+2; i++)
//				temp[i] = Ftp[i]; //xx[my_mesh->My2][i+my_mesh->xs].fn[0];
//			MPI_Send(temp, send, MPI_DOUBLE, rank1, 0, commy);
//
//			// domain with trap-passing boundary receive the non-local flux
//		} else if (my_mesh->My1>=my_mesh->ys && my_mesh->My1<(my_mesh->ys+my_mesh->ym)) // j=My1, trap domain with trap-passing boundary 1{
//			MPI_Recv(Ftp.data(), recv, MPI_DOUBLE, rank2, 0, commy, MPI_STATUS_IGNORE);
//
//		//		if (my_mesh->My1>=my_mesh->ys && my_mesh->My1<(my_mesh->ys+my_mesh->ym)) { // j=My1, trap domain with trap-passing boundary 1{
//		//			MPI_Recv(Ftp.data(), recv, MPI_DOUBLE, rank2, 0, commy, MPI_STATUS_IGNORE);
//		//
//		//			// now setting non-local t-p flux
//		//			for(int i=my_mesh->xs; i<my_mesh->xs+my_mesh->xm; i++) {
//		//				vip12 = my_mesh->xx[i];
//		//				coef = -(E/vip12) + sr/sqrt(1.+vip12*vip12)*ba_ctx.xic*tok;
//		//				//Ftp temporaly stores the distribution at the first point in the +1 passing region
//		//				f = Ftp[i-my_mesh->xs]; // upstream value
//		//				// f = 0.5*(xx[j][i].fn[0] + Ftp[i-my_mesh->xs]);
//		//				Ftp[i-my_mesh->xs] = (1. - ba_ctx.xic*ba_ctx.xic) * ( coef * xx[my_mesh->My1][i].fn[0] - tok * dTv[i-my_mesh->xs] * (Ftp[i-my_mesh->xs] - xx[my_mesh->My1][i].fn[0])/(my_mesh->yy[my_mesh->My2] + my_mesh->yy[my_mesh->My1]) );
//		//
//		//				if (dw_) {   // with quasilinear diffusion
//		//					double v = vip12/sqrt(1.0+vip12*vip12);
//		//					double dfdp, D, fluxe, fluxw, flux;
//		//
//		//					int idx1 = (i-my_mesh->xs+1) + (j-my_mesh->ys+1)*(my_mesh->xm+2);
//		//					int idx2 = (i-my_mesh->xs+1) + (j+1-my_mesh->ys+1)*(my_mesh->xm+2);
//		//
//		//					// d^2f/dxi^2
//		//					D = 0.5*(Dw.xixi[idx1] + Dw.xixi[idx2]);
//		//					fluxe = -(D/(vip12*vip12)) * (f - xx[my_mesh->My1][i].fn[0])/(my_mesh->yy[my_mesh->My2]+ my_mesh->yy[my_mesh->My1]);
//		//
//		//					// the off-diagonal coefficient
//		//					D = 0.5*(Dw.pxi[idx1] + Dw.pxi[idx2]);
//		//
...
//		//					if (i > 0) {
//		//						if (D<0) {
//		//							fluxe -= D/vip12 * (xx[my_mesh->My1+1][i+1].fn[nn] - xx[my_mesh->My1+1][i].fn[nn])/(my_mesh->xx[i+1] - my_mesh->xx[i]);
//		//							fluxw -= D/vip12 * (xx[my_mesh->My1][i].fn[nn] - xx[my_mesh->My2][i-1].fn[nn])/(my_mesh->xx[i] - my_mesh->xx[i-1]);
//		//						} else {
//		//							fluxe -= D/vip12 * (xx[my_mesh->My1+1][i].fn[nn] - xx[my_mesh->My1+1][i-1].fn[nn])/(my_mesh->xx[i] - my_mesh->xx[i-1]);
//		//							fluxw -= D/vip12 * (xx[my_mesh->My1][i+1].fn[nn] - xx[my_mesh->My1][i].fn[nn])/(my_mesh->xx[i+1] - my_mesh->xx[i]);
//		//						}
//		//
//		//						if (fluxe*fluxw > 0)
//		//							flux += 2.*fluxw* fabs(fluxe)/(fabs(fluxe)+fabs(fluxw));
//		//
//		//					} else if (i == 0) {
//		//						fluxe -= D/vip12 * 0.5*(xx[my_mesh->My2+1][i+1].fn[nn] + xx[my_mesh->My2][i+1].fn[nn] - xx[my_mesh->My2+1][i].fn[nn] - xx[my_mesh->My2][i].fn[nn]) / (my_mesh->xx[i+1] - my_mesh->xx[i]);
//		//						flux = fluxe;
//		//					}
//		//
//		//					// convective flux from quasilinear term due to using f*p
//		//					if (D>0)
//		//						flux = D/(vip12*vip12) * 0.5*(xx[my_mesh->My2][i].fn[nn] + xx[my_mesh->My2][i].fn[nn]);
//		//					else
//		//						flux = D/(vip12*vip12) * 0.5*(xx[my_mesh->My2+1][i].fn[nn] + xx[my_mesh->My2+1][i].fn[nn]);
//		//				}
//		//
//		//				Ftp[i-my_mesh->xs] += flux * (1. - ba_ctx.xic*ba_ctx.xic);
//		//			}
//		//
//		//		}
//		delete [] temp;
//
//	} else { // trap-passing boundary on the same rank
//
//


/*---old stuff in compute flux------*/
//			if (j == my_mesh->My2) { // trap->passing + (above xitp)
//				flux = sr/sqrt(1.+vip12*vip12)*ba_ctx.xic * (ba_ctx.zeta2_jp1[my_mesh->My1]) * Ftp[i-my_mesh->xs+1];
//				flux +=	- (ba_ctx.zeta2_jp1[my_mesh->My1]) * dTv[i-my_mesh->xs] * (xx[j][i].fn[0] - Ftp[i-my_mesh->xs+1])/(xijp12 - xij);
//
//				ff[j][i].fn[nn] += (Fxi(nn, i, j, xx) - flux) /(xijp1-xij)/ba_ctx.zeta1_jp12[j];
//
//			} else if (j == my_mesh->My1+1) { // traped; notice the factor of 2 before zeta_2 at -xic  (above -xitp)
//				flux = sr/sqrt(1.+vip12*vip12)*(-ba_ctx.xic) * (2.*ba_ctx.zeta2_jp1[my_mesh->My1]) * Ftp[i-my_mesh->xs+1]; // lower edige flux
//				flux +=	- (2.*ba_ctx.zeta2_jp1[my_mesh->My1]) * dTv[i-my_mesh->xs] * (xx[j][i].fn[0] - Ftp[i-my_mesh->xs+1])/(xijp12 - xij);
//
//				ff[j][i].fn[nn] += (Fxi(nn, i, j, xx) - flux) /(xijp1-xij)/ba_ctx.zeta1_jp12[j];
//
//			} else if (j == my_mesh->My1) { // pass->trap  (below -xitp)
//				flux = sr/sqrt(1.+vip12*vip12)*(-ba_ctx.xic) * (ba_ctx.zeta2_jp1[j]) * Ftp[i-my_mesh->xs+1]; // upper edge flux
//				flux +=	- (ba_ctx.zeta2_jp1[j]) * dTv[i-my_mesh->xs] * (Ftp[i-my_mesh->xs+1] - xx[j][i].fn[0])/(xijp1 - xijp12);
//
//				ff[j][i].fn[nn] += (flux - Fxi(nn, i, j-1, xx)) /(xijp1-xij)/ba_ctx.zeta1_jp12[j];
