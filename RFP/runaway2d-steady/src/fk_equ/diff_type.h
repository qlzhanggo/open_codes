...
class flux_op
{
 public:
  ...
  // upwind scheme: from upstream to downstream Ur, Uu, Ud with non-uniform mesh (cell-edged)
  double operator() (const PetscScalar &Ur, const PetscScalar &Uu, const PetscScalar &Ud, const double&xr, const double&xu, const double&xd, const UWtype type) 
  {
	...
	PetscPrintf(PETSC_COMM_WORLD,"not supported yet!\n");
      ...
      //this is only for uniform meshes
      if (Ur != Ud)
	fnorm = (Uu - Ur) / (Ud - Ur);
      else
	fnorm = -1.0;
	...
	PetscPrintf(PETSC_COMM_WORLD,"not supported yet!\n");
        abort();
    /*
      r1 = (xd - xu)/(xu - xr); r2 = (x-xu)/(xu-xr);
      q1 = 0.25*(2.+r1); q2 = 0.25*(2.+r1)/(1.+r1);
	...
	if (fnorm<=0. || fnorm>=1.)
	  ...
	  if (fnorm>0.0 && fnorm < q2/(10.-q1))
	    fnorm *= 10.;
	  else if (fnorm >= q2/(10.-q1) && fnorm < q2/(1.+r2-q1))
	    fnorm = q1*fnorm + q2;
	  else if (fnorm >= q2/(1.+r2-q1) && fnorm <= 1./(1+r2))
	    fnorm *= (1.+r2);
	  else if (fnorm >= 1./(1+r2) && fnorm < 1.0)
	    fnorm = 1.;

	  return fnorm * Ud + (1.-fnorm)*Ur;
	}
      } else
	return Uu;
        */
	...
	PetscPrintf(PETSC_COMM_WORLD,"not supported yet!\n");
        abort();
     /*
      r1 = (xu - xr)/(xd - xr); r2 = (x-xr)/(xd-xr);
      q1 = r2/(r1-r1*r1); q2 = r2-r1;
	...
	if (fnorm<=0. || fnorm>=1.)
	  ...
	  if (fnorm>0.0 && 3.*fnorm < r1)
	    fnorm *= (q1*(1. - 3*r1 + 2.*r2));
	  else if (3.*fnorm >= r1 && fnorm < r1*(1.+r2-r1)/r2)
	    fnorm = q1*(1.-r2)*fnorm + q1*r1*(r2-r1);
	  else if (fnorm >= r1*(1.+r2-r1)/r2 && fnorm < 1.)
	    fnorm = 1.;
	...
	if (fnorm<=0. || fnorm >= 1.)
	  ...
	  if (fnorm>0.0 && fnorm < r1)
	    fnorm *= (r2/r1);
	  else if (fnorm >= r1 && fnorm < 1.)
	    fnorm = fnorm*(1.-r2)/(1.-r1) + (r2-r1)/(1.-r1);
