...
//Compute cell-centered diffusion coefficients at xx
void Dwp_slab::setup(double w0_, double k10_, double k20_, double dk10_, double wpe)
{
    ...
    //this is a simple check to make sure diffusion has a symmetric positive definite matrix
    //it does not impact the calculation 
	...
	for (j=my_mesh->ys-1; j<my_mesh->ys+my_mesh->ym+1; j++) { //xi
		...
		for (i=my_mesh->xs-1; i<my_mesh->xs+my_mesh->xm+1; i++) { //p
           ...
           //here I guess there are two ghost points, so I will not output them
    ...
    VecDestroy(&XIXI);
        ...
        if (false)
        {
	        ...
	        std::ofstream ofile;
            //ofile.open("Dwp.dat", std::ios::out | std::ios::binary);
            ofile.open("Dwp.dat");
	        if (ofile.is_open()) {
                int xm_=my_mesh->xm;
                int ym_=my_mesh->ym;
...
//similar with setup but compute upper cell-faced values at xf
void Dwp_slab::setupFace(double w0_, double k10_, double k20_, double dk10_, double wpe)
{
	...
	PetscPrintf(PETSC_COMM_WORLD,"Setting up the slab geometry wave-particle diffusion operator (cell-face values)\n");
	...
	for (j=my_mesh->ys-1; j<my_mesh->ys+my_mesh->ym; j++) { //xi
		...
		xijp1 = (j==-1) ? (-1.0): (my_mesh->yf[j]);

		for (i=my_mesh->xs-1; i<my_mesh->xs+my_mesh->xm; i++) { //p

			int idx = (i-my_mesh->xs+1) + (j-my_mesh->ys+1)*(my_mesh->xm+1); // index of the diffusion coefficient matrix

			pp[idx] = 0;
			pxi[idx] = 0;
			xixi[idx] = 0;

			vip1 = (i==-1) ? (my_mesh->xmin) : (my_mesh->xf[i]);

			D_tmp.fill(0.);
			normal(es, m, ev, vip1, xijp1, D_tmp);
