 ...
 * */
fk_equ::fk_equ(mesh *mesh_, Field_EQU *E_Field_, char* param_file) : my_mesh(mesh_), E_Field(E_Field_), Z(1.0), rho(0.03), ctr(10.0), E0(1.0), sr(0.01429),
                    asp(0.3), q(1.0), ra(0.5), beta(0.0667), bc_type(0), diff_type(QUICK),
					ne(1.0), vte(0.01), Dpp(0.5e-4), dw_(false), ko_(false),
					imp_part(0), partial_(false), imp_atom(18.0), imp_charge(1.0), imp_I(7.8), imp_a(4.5)
{
    ...
    state.Z = Z; state.E = E0; state.alpha = sr;
    E_Field->set_state(state);
    ...
    PetscPrintf(PETSC_COMM_WORLD,"E0 (initial) = %lg\n", state.E);
    ...
    for (i=my_mesh->xs-1; i<my_mesh->xs+my_mesh->xm; i++) {
      if (i == -1 )
        vip1 = my_mesh->xmin;  // cell upper face
      else if (i<(my_mesh->Mx))
        vip1 = my_mesh->xf[i];
    
      //what are the lambda_ee and lambda_ei doing?? -QT
    ...
    for (i=my_mesh->xs; i<my_mesh->xs+my_mesh->xm; i++) {
      vip12 = my_mesh->xx[i];  //cell center
      dTv[i-my_mesh->xs] = 0.5*sqrt(1+square1(vip12))/cubic(vip12) * ( state.Z * Lambda_ei(vip12)
...
void fk_equ::update_dTv(double &dZ)
{
